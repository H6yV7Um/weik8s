...
def job_util_path():
    ...
    return os.path.abspath("/data0/control_center/weibox/yarn-weibox" + "/tools/utils")
...
def submit_script_generator(_job_key = "",
    ...
    from algorithm.lda_parameter_filter import lda_json_convert_to_paramters
    import json
    _key = "result.json"
    rst_json = get_args.get_args(_job_key, _key)
    _run_command = lda_json_convert_to_paramters(json.dumps(rst_json), _job_key=_job_key)
